...
interface LedgerRow {
  ...
  stampClass: string;
  tier: 'TIER_1' | 'TIER_2';
  district: 'DELHI' | 'MUMBAI' | 'BANGALORE' | 'LUCKNOW';
}

const LEDGER_ROWS: LedgerRow[] = [
  { id: '#AQ-294-81', timestamp: '2026.07.21 14:32:01', category: 'Property Trespass', location: '400 BLOCK OF KEARNY ST', provenance: 'VERIFIED', stampClass: 'stamp-verified', tier: 'TIER_2', district: 'MUMBAI' },
  { id: '#AQ-294-85', timestamp: '2026.07.21 14:45:12', category: 'Noise Disturbance', location: '100 BLOCK OF MARKET ST', provenance: 'PERIODIC', stampClass: 'stamp-periodic', tier: 'TIER_1', district: 'MUMBAI' },
  { id: '#AQ-294-89', timestamp: '2026.07.21 15:01:44', category: 'Vehicle Recovery', location: '2200 BLOCK OF VALENCIA ST', provenance: 'VERIFIED', stampClass: 'stamp-verified', tier: 'TIER_2', district: 'DELHI' },
  { id: '#AQ-294-92', timestamp: '2026.07.21 15:22:10', category: 'System Anomaly', location: 'BLOCK MASKED - INFRASTRUCTURE', provenance: 'PERIODIC', stampClass: 'stamp-periodic', tier: 'TIER_1', district: 'BANGALORE' },
  { id: '#AQ-294-98', timestamp: '2026.07.21 16:05:00', category: 'Public Safety Signal', location: '100 BLOCK OF POST ST', provenance: 'VERIFIED', stampClass: 'stamp-verified', tier: 'TIER_2', district: 'DELHI' },
  { id: '#AQ-295-01', timestamp: '2026.07.21 16:12:30', category: 'Unscheduled Maintenance', location: '300 BLOCK OF MISSION ST', provenance: 'PERIODIC', stampClass: 'stamp-periodic', tier: 'TIER_1', district: 'LUCKNOW' },
];

const ALL_FEEDS = [
  {
    districtCode: 'MUMBAI',
    district: 'DISTRICT 04 (MUMBAI)',
    change: '+4.2%',
    isUp: true,
  ...
  {
    districtCode: 'DELHI',
    district: 'DISTRICT 11 (DELHI NCR)',
    change: '-1.8%',
    isUp: false,
  ...
  },
  {
    ...
    district: 'DISTRICT 02 (BANGALORE)',
    change: '+6.5%',
    isUp: true,
    title: 'Cyber Fraud Spike',
    forecast: '7-Day Forecast: Rapid Ascent',
    svgPath: 'M0,20 Q20,10 40,15 T70,5 T90,0',
...
export default function IncidentsLedgerPage() {
  ...
  const filteredRows = LEDGER_ROWS.filter(r => {
    const matchesSearch = `${r.id} ${r.category} ${r.location}`.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesTier = selectedTier === 'ALL_TIERS' || r.tier === selectedTier;
    const matchesDistrict = selectedDistrict === 'ALL_REGIONS' || r.district === selectedDistrict;
    const matchesCategory = selectedCategory === 'ALL_CATEGORIES' ||
      (selectedCategory === 'PROPERTY' && r.category.toLowerCase().includes('property')) ||
      (selectedCategory === 'NOISE' && r.category.toLowerCase().includes('noise')) ||
      (selectedCategory === 'VEHICLE' && r.category.toLowerCase().includes('vehicle')) ||
      (selectedCategory === 'SYSTEM' && r.category.toLowerCase().includes('system')) ||
      (selectedCategory === 'PUBLIC_SAFETY' && r.category.toLowerCase().includes('public'));

    return matchesSearch && matchesTier && matchesDistrict && matchesCategory;
  });

  const filteredFeeds = ALL_FEEDS.filter(f => {
    const matchesDistrict = selectedDistrict === 'ALL_REGIONS' || f.districtCode === selectedDistrict;
    const matchesCategory = selectedCategory === 'ALL_CATEGORIES' || f.categoryKey === selectedCategory;
    return matchesDistrict && matchesCategory;
  });

  // Dynamic Metrics Recalculation
  const confidenceScore = (98.1 - (filteredRows.length * 0.4)).toFixed(1);
  const activeSensorsCount = (14204 + (filteredRows.length * 125)).toLocaleString();
  const aggRate = (0.942 + (filteredRows.length * 0.01)).toFixed(3);
    ...
    <LedgerShell>
      <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto font-sans selection:bg-cobalt selection:text-white">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[280px_1fr] gap-6">
          ...
          {/* Left Column: Dynamic Intelligence Feed Cards */}
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-mono text-xs font-bold uppercase tracking-widest text-ink-soft">
                Intelligence Feed
              </h2>
              <span className="font-mono text-[9px] text-cobalt font-bold">({filteredFeeds.length} FEEDS)</span>
            </div>

            {filteredFeeds.length === 0 ? (
              <div className="p-4 rounded border border-hairline bg-paper-raised text-center text-xs text-ink-soft font-mono">
                No feeds match active filters.
              </div>
            ) : (
              filteredFeeds.map((feed, idx) => (
                <div key={idx} className="ledger-panel rounded p-4 border border-hairline bg-paper-raised transition-all hover:border-cobalt">
            ...
            <Link 
              href="/incidents/new" 
              className="w-full py-3 px-4 rounded bg-ink text-white font-mono text-xs font-bold tracking-wider uppercase flex items-center justify-center gap-2 hover:bg-black transition-all cursor-pointer shadow-sm"
            >
              + NEW INCIDENT
            </Link>
          </div>

          {/* Right Column: Dynamic Dropdown Filters & Main Ledger Table */}
          <div className="space-y-6">
            
            {/* Top Dynamic Dropdowns Header Bar */}
            <div className="flex flex-wrap gap-4 items-center justify-between bg-paper-raised p-4 border border-hairline rounded shadow-sm">
              <div className="flex flex-wrap items-center gap-3 font-mono text-xs">
                
                {/* 1. TIER RANGE Dropdown */}
                <div>
                  <label className="block text-[9px] text-ink-soft uppercase mb-1 font-bold">TIER RANGE</label>
                  <select 
                    value={selectedTier} 
                ...
                {/* 2. CATEGORY Dropdown */}
                <div>
                  <label className="block text-[9px] text-ink-soft uppercase mb-1 font-bold">CATEGORY</label>
                  <select 
                  ...
                  >
                    ...
                    <option value="VEHICLE">VEHICLE_RECOVERY</option>
                    <option value="SYSTEM">SYSTEM_ANOMALY</option>
                    <option value="PUBLIC_SAFETY">PUBLIC_SAFETY_SIGNAL</option>
                  </select>
                </div>

                {/* 3. DISTRICT Dropdown */}
                <div>
                  <label className="block text-[9px] text-ink-soft uppercase mb-1 font-bold">DISTRICT</label>
                  <select 
                  ...
                  >
                    ...
                    <option value="BANGALORE">BANGALORE_URBAN</option>
                    <option value="LUCKNOW">LUCKNOW_CENTRAL</option>
                  </select>
                </div>
            ...
            <div className="ledger-panel rounded border border-hairline bg-paper-raised overflow-hidden">
                ...
                <table className="w-full text-left font-mono text-xs">
                  ...
                  <tbody className="divide-y divide-hairline">
                    {filteredRows.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="px-5 py-8 text-center text-ink-soft italic">
              ...
              <div className="p-4 border-t border-hairline bg-paper/40 flex items-center justify-between text-[11px] font-mono text-ink-soft">
                <span>SHOWING {filteredRows.length} OF 124 VERIFIED RECORDS</span>
            ...
            {/* Bottom Row: Map Thumbnail + Dynamic Metadata Analysis Card */}
            <div className="grid grid-cols-1 md:grid-cols-[1fr_300px] gap-6">
              ...
              <div className="ledger-panel rounded border border-hairline bg-[#E2DFD5] h-48 relative overflow-hidden flex items-center justify-center">
                <span className="absolute top-3 left-3 bg-paper-raised px-2.5 py-1 border border-hairline rounded text-[10px] font-mono font-bold text-ink uppercase">
                  REGIONAL HOTSPOT OVERLAY ({selectedDistrict})
                </span>
              ...
              {/* Dynamic Metadata Analysis Card */}
              <div className="ledger-panel rounded p-4 border border-hairline bg-paper-raised flex flex-col justify-between font-mono text-xs shadow-sm">
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-ink">{aggRate}ms</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-ink-soft">Confidence Score</span>
                      <span className="font-bold text-stamp-green">{confidenceScore}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-ink-soft">Active Sensors</span>
                      <span className="font-bold text-ink">{activeSensorsCount}</span>
                    </div>
                  </div>
                </div>

                <p className="text-[9px] text-ink-soft italic leading-tight mt-4 pt-2 border-t border-hairline">
                  The information contained in this ledger has been cross-referenced with regional field intelligence. Timestamp integrity is verified by central datum node.
                </p>
              </div>
